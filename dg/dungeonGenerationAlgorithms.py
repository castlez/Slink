...
from libtcodpy import map_set_properties, FONT_TYPE_GREYSCALE, console_new, console_set_default_background, \
	console_clear, console_set_custom_font, gray, white, console_is_window_closed, map_new, FONT_LAYOUT_TCOD, \
	console_put_char_ex, KEY_ESCAPE, LEFT, path_size, Color, black, console_set_default_foreground, console_init_root, \
	KEY_1, path_delete, KEY_0, KEY_3, KEY_2, console_blit, KEY_5, KEY_4, KEY_7, path_compute, KEY_6, KEY_8, \
	console_check_for_keypress, console_flush, KEY_SPACE, console_print_ex, BKGND_NONE, Key, path_new_using_map
import random
...
class UserInterface:
	def __init__(self):
		...
		console_init_root(SCREEN_WIDTH, SCREEN_HEIGHT, 'Roguelike Dungeon Comparison', False) #TODO: Change Game Name
		self.con = console_new(SCREEN_WIDTH, SCREEN_HEIGHT)
		
		self.textBox = console_new(SCREEN_WIDTH, TEXTBOX_HEIGHT)
	...
	def mainLoop(self):
		...
		while not console_is_window_closed():
			...
			#Input
			keyboard = console_check_for_keypress()
	...
	def handleInput(self,keyboard):
		if (keyboard.vk	== KEY_ESCAPE):
			return True #Exit Program	

		if (keyboard.vk == KEY_SPACE):
		...
		for y in range(MAP_HEIGHT):
			for x in range(MAP_WIDTH):
				if self.map.level[x][y] == 1:
					console_put_char_ex(self.con, x, y, '#', self.color_light_wall_fore, self.color_light_wall_back)
				else:
					console_put_char_ex(self.con, x, y, '.', self.color_light_ground_fore, self.color_light_ground_back)
	
		#TODO: Print Instructions to Screen
		self.renderTextBox()
		# ==== Blit Console to Screen ====
		console_blit(self.con, 0, 0, MAP_WIDTH, MAP_HEIGHT, 0, 0, 0)

	def renderTextBox(self):
		console_set_default_background(self.textBox, black)
		console_set_default_foreground(self.textBox, white)
		console_clear(self.textBox)
		...
		for key in keys:
			...
			console_print_ex(self.textBox, x, y, BKGND_NONE, LEFT,
		...
		console_blit(self.textBox, 0, 0, SCREEN_WIDTH, TEXTBOX_HEIGHT, 0, 0, SCREEN_HEIGHT - TEXTBOX_HEIGHT)
...
class ColorScheme():
	...
	#DEFAULT
	BLUE = [
	Color(100, 100, 100),	# color_light_wall_fore
	Color(50, 50, 150),	# color_light_wall_back
		gray, 				# color_light_ground_fore
	Color(10, 10, 10) 	# color_light_ground_back
	]
	_scheme.append(BLUE)

	MAUVE = [
	Color(50, 50, 50),	# color_light_wall_fore
	Color(204, 153, 255),	# color_light_wall_back
		gray, 				# color_light_ground_fore
	Color(10, 10, 10) 	# color_light_ground_back
	]
	...
	GRAYSCALE = [
		black, 				# color_light_wall_fore
		gray,			# color_light_wall_back
		white, 				# color_light_ground_fore
		black  # color_light_ground_back
	]
	_scheme.append(GRAYSCALE)

	TEXTONLY = [
		white, 				# color_light_wall_fore
		black,			# color_light_wall_back
		white, 				# color_light_ground_fore
		black  # color_light_ground_back
	]
...
class RoomAddition:
	...
	def addShortcuts(self,mapWidth,mapHeight):
		...
		libtcodMap = map_new(mapWidth, mapHeight)
				...
				for y in range(-1,2):
					if x != 0 or y != 0: # Exclude the center tile
						...
						# run pathfinding algorithm between the two points
							...
							pathMap = path_new_using_map(libtcodMap)
							path_compute(pathMap, floorX, floorY, newX, newY)
							distance = path_size(pathMap)
	...
	def recomputePathMap(self,mapWidth,mapHeight,libtcodMap):
		for x in range(mapWidth):
			for y in range(mapHeight):
				if self.level[x][y] == 1:
					map_set_properties(libtcodMap, x, y, False, False)
				else:
					map_set_properties(libtcodMap, x, y, True, True)
